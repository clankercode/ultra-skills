...
8. **Dispatch brief template** — every brief MUST contain:
   ...
   - **Skills to load** — workers load `ultra-test-driven-development` (RED-GREEN-REFACTOR discipline, Iron Law) and `ultra-writing-tests` (test-writing craft: fast-test preference, DI-seam discipline, contract smoke tests) as standard. Workers MUST also load `ultra-code-standards` for LOC limits, duplication rules, and quality bar. **Domain references (conditional):** if the worker's SUT is a TUI (Bubble Tea, Ratatui, Textual, Ink, tcell/tview, Rich), the brief MUST also direct the worker to load `ultra-tui-iteration` for headless-harness selection, snapshot/golden assertion default, and the agent-readable feedback loop — without it, workers default to substring assertions on `View()` and ship untested rendering. Prefer fast tests per `ultra-writing-tests` — unit <100ms, integration <1s, context-dependent.
