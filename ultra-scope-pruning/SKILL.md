...
4. **Emit a verdict per feature.** One of: **KEEP** (required for anchor job), **DOWNSCOPE** (anchor needs a thin version, not the full thing — name the thin version), **DEFER** (useful but not for v1 — name the trigger that would revive it), **CUT** (not aligned with anchor), **INFEASIBLE** (appears technically impossible or impractical — requires research confirmation). Each verdict carries a one-sentence rationale. Write verdicts into `FEATURES.md` next to each feature. For any INFEASIBLE verdict: before finalizing, dispatch a research subagent (via `ultra-plan-research`) to investigate whether the feature is truly infeasible or whether creative approaches exist. If confirmed infeasible, surface to user as P0 in INTERVIEW_QUEUE.md with evidence and alternative approaches that achieve the underlying goal. Write a `blocked — infeasible` ADR stub in DECISIONS.md.
